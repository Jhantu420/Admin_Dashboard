...
function TestingAdmin() {
  ...
  const fetchAdmins = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3001/testinguser"
      );
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(`http://localhost:3001/testinguser/delete/${id}`);
  ...
  return (
    <div>
      <h2 className="heading">◤✧ 𝕿𝖊𝖘𝖙𝖎𝖓𝖌 𝕬𝖉𝖒𝖎𝖓 ✧◥</h2>
            ...
            <tbody>
              {filteredAdmins.map((admin) => (
                <tr key={admin.id}>
                  ...
                  <td>
                    <Link to={`/updateTestinguser/${admin.id}`}>
