...
function DataPreAdmin() {
  ...
  const fetchAdmins = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3001/datapreparationuser"
      );
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(`http://localhost:3001/datapreparationuser/delete/${id}`);
  ...
  return (
    <div>
      <h2 className="heading">◤✧ 𝕯𝖆𝖓𝖙𝖆 𝕻𝖗𝖊𝖕𝖆𝖗𝖆𝖙𝖎𝖔𝖓 𝖆𝖉𝖒𝖎𝖓 ✧◥</h2>
            ...
            <tbody>
              {filteredAdmins.map((admin) => (
                <tr key={admin.id}>
                  ...
                  <td>
                    <Link to={`/updateDataPreUser/${admin.id}`}>
