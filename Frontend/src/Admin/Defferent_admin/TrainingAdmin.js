...
function TrainingAdmin() {
  ...
  const fetchAdmins = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3001/traininguser"
      );
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(`http://localhost:3001/traininguser/delete/${id}`);
  ...
  return (
    <div>
      <h2 className="heading"> ◤✧ 𝕿𝖗𝖆𝖎𝖓𝖎𝖓𝖌 𝕬𝖉𝖒𝖎𝖓 ✧◥</h2>
            ...
            <tbody>
              {filteredAdmins.map((admin) => (
                <tr key={admin.id}>
                  ...
                  <td>
                    <Link to={`/updateTrainingUser/${admin.id}`}>
